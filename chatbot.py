...
async def chat_endpoint(request: Request):
    ...
    history = []  # Optionally, parse history from frontend if needed
    if not message:
        return JSONResponse({"response": "No message provided."}, status_code=400)
    # Collect the full response from the generator
    response_gen = stream_response(message, history)
    full_response = ""
    try:
        for part in response_gen:
            full_response = part  # always take the latest yielded value (the full message so far)
    except Exception:
        pass
